...
my $data = [
  ...
  [ 'strongest_team',       'The Giants',  ,     'The Tall People', 'The Bigger, stronger, faster' ],
  [ 'weakest_team',       'teams under indictment',       'the washouts', 'the rodent mascots' ],
];

my @region = ( 'east', 'west', 'south', 'midwest' );    #4 different brackets
...
while ( $trials > 0 ) {
  ...
  for my $r (@region) {
    ...
    my $ref_calc = calc_winners( $ref_bracket, $r );
    dd $ref_calc;
    my @sieger = @$ref_calc;
    say "winners are @sieger";
    my $anzahl = scalar @sieger; 
